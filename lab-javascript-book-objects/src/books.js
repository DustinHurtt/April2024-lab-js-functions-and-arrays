...
book = {
  ...
  NOTES:
    ...
    book.details.description = 'The Art of Learning takes readers through Waitzkins unique journey to excellence. He explains in clear detail how a well-thought-out, principled approach to learning is what separates success from failure.'
...
function booksByAuthor(dictionary) {
  ...
  for (const author in dictionary) {
    dictionary[author].forEach(book => {
      bookArray.push({title: book[0], pages: book[1], author: author})
    })
  }
  return bookArray
...
function averagePageCount(arrayOfBooks) {
  ...
  return Number(pageCount/arrayOfBooks.length);
}
